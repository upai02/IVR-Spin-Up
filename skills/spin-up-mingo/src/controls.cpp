...
void group_stop(pros::motor_brake_mode_e b) {
    ...
    left_back_top_mtr.set_brake_mode(b);
    left_back_top_mtr.brake();
    right_back_top_mtr.set_brake_mode(b);
    right_back_top_mtr.brake();
    left_back_bottom_mtr.set_brake_mode(b);
    left_back_bottom_mtr.brake();
    right_back_bottom_mtr.set_brake_mode(b);
    right_back_bottom_mtr.brake();
}

void move_with_assigned_speed(double xVel, double yVel, double turnVel) {
    left_front_mtr.move_velocity(yVel + xVel - turnVel);
    left_back_top_mtr.move_velocity(yVel - xVel - turnVel);
    left_back_bottom_mtr.move_velocity(yVel - xVel - turnVel);
    right_front_mtr.move_velocity(yVel - xVel + turnVel);
    right_back_top_mtr.move_velocity(yVel + xVel + turnVel);
    right_back_bottom_mtr.move_velocity(yVel + xVel + turnVel);
}
...
void wannabeSwerve() {
    ...
    left_back_top_mtr.move(power + desiredTranslation);
    left_back_bottom_mtr.move(power + desiredTranslation);
    right_front_mtr.move(-power + desiredTranslation);
    right_back_top_mtr.move(-power + desiredTranslation);
    right_back_bottom_mtr.move(-power + desiredTranslation);
}

void shoot() {
    catapult.move_relative(360, 100);
}
